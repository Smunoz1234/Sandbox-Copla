...
if (($type == 1) && ($Serie != "") && ($Sucursal != "")) {
	...
	// echo "SELECT WhsCode, WhsName FROM uvw_tbl_SeriesSucursalesAlmacenes WHERE $WhereAlmacen GROUP BY WhsCode, WhsName ORDER BY WhsName";
...
if (($type == 1) && ($Serie != "") && ($Sucursal != "")) {
	...
	// echo "<br>SELECT ToWhsCode, ToWhsName FROM uvw_tbl_SeriesSucursalesAlmacenes WHERE $WhereToAlmacen GROUP BY ToWhsCode, ToWhsName ORDER BY ToWhsName";
						...
						while ($row = sqlsrv_fetch_array($SQL)) {
							...
							<tr class="trContenido">
								...
								<td><input size="15" type="text" id="Quantity<?php echo $i; ?>" name="Quantity[]"
										...
										onKeyPress="return justNumbers(event,this.value);" <?php if ($row['LineStatus'] == 'C' || $Estado == 2 || (!PermitirFuncion(1201))) {
											echo "readonly";
										} ?>
										onFocus="focalizarValores(this)"></td>
								...
								<td><input size="15" type="text" id="OpenQty<?php echo $i; ?>" name="OpenQty[]"
										class="form-control" value="<?php echo number_format($row['OpenQty'], 2); ?>"
										onKeyUp="revisaCadena(this);" onKeyPress="return justNumbers(event,this.value);"
										readonly></td>
									...
									<select id="PrjCode<?php echo $i; ?>" name="PrjCode[]" class="form-control select2"
										...
										<?php while ($row_Proyecto = sqlsrv_fetch_array($SQL_Proyecto)) { ?>
											<option value="<?php echo $row_Proyecto['IdProyecto']; ?>" <?php if ((isset($row['PrjCode'])) && (strcmp($row_Proyecto['IdProyecto'], $row['PrjCode']) == 0)) {
												   echo "selected=\"selected\"";
											   } ?>><?php echo $row_Proyecto['IdProyecto'] . "-" . $row_Proyecto['DeProyecto']; ?></option>
										<?php } ?>
									</select>
								</td>
								...
								<td><input size="50" type="text" id="FreeTxt<?php echo $i; ?>" name="FreeTxt[]"
										...
										maxlength="100" <?php if ($row['LineStatus'] == 'C' || $Estado == 2 || (!PermitirFuncion(1201))) {
											echo "readonly";
										} ?>></td>
								...
								<td>
									<input size="15" type="text" id="LineTotal<?php echo $i; ?>" name="LineTotal[]"
										...
										onBlur="CalcularTotal(<?php echo $i; ?>, false);" onFocus="focalizarValores(this)">
								</td>
