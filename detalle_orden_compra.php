    ...
    while ($row = sqlsrv_fetch_array($SQL)) {
        ...
        if ($flag || (($row['LineNum'] != 0) || ($row['TreeType'] != "T"))) {?>
			...
			<?php foreach ($array_Dimensiones as &$dim) {?>
				...
				<td>
					<select id="OcrCode<?php echo $OcrId . $i; ?>" name="OcrCode<?php echo $OcrId; ?>[]" class="form-control select2" onChange="ActualizarDatos('OcrCode<?php echo $OcrId; ?>',<?php echo $i; ?>,<?php echo $row['LineNum']; ?>);" <?php if ($row['LineStatus'] == 'C' || (!PermitirFuncion(702))) {echo "disabled='disabled'";}?>>
						<option value="">(NINGUNO)</option>
