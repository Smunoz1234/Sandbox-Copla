...
if (isset($_POST['P']) && ($_POST['P'] != "")) { //Grabar Salida de inventario
	...
	try {
		...
		if ($SQL_CabeceraTrasladoInv) {
			...
			$Cabecera = array(
				"crear_salida_inventario" => PermitirFuncion(1214),
				// SMM, 22/02/2023
				"id_serie_salida_inventario" => (PermitirFuncion(1214) ? intval(ObtenerVariable("IdSerieSalidaInvPorDefecto")) : null),
				// SMM, 28/02/2023
				"CDU_nombre_firma_recibe" => ($row_Cab['NombreRecibeFirma'] ?? ""),
				// SMM, 20/02/2023
				"CDU_CC_firma_recibe" => ($row_Cab['CedulaRecibeFirma'] ?? ""),
				// SMM, 20/02/2023
				...
				"docentry_documento" => $row_Cab['DocEntry'] ?? 0,
				// SMM, 01/12/2022
	...
	if(dato!=""){
		if((cardcode!="")&&(almacen!="")&&(almacendestino!="")){
			remote=open('buscar_articulo.php?concepto=${conceptoSalida}&dato='+dato+'&cardcode='+cardcode+'&prjcode='+proyecto+'&whscode='+almacen+'&towhscode='+almacendestino+'&doctype=<?php if ($edit == 0) {
				...
				echo "0";
			} ?>&tipodoc=3&dim1='+dim1+'&dim2='+dim2+'&dim3='+dim3+'&dim4='+dim4+'&dim5'+dim5,'remote',"width=1200,height=500,location=no,scrollbars=yes,menubars=no,toolbars=no,resizable=no,fullscreen=no,directories=no,status=yes,left="+posicion_x+",top="+posicion_y+"");
			remote.focus();
		}else{
			Swal.fire({
				title: "¡Advertencia!",
...
<script type="text/javascript">
	$(document).ready(function() {//Cargar los combos dependiendo de otros
		$("#CardCode").change(function(){
			...
			<?php if ($dt_SS == 0 || ($dt_DR == 1 && $sw_error == 0)) { //Para que no recargue las listas cuando vienen de una solicitud de salida.?>
					$.ajax({
						type: "POST",
						...
						success: function(response){
							...
							<?php if (($edit == 0) && ($ClienteDefault != "") && ($dt_SS == 0)) { ?>
...
// Actualización dinámica, llega hasta aquí.
	...
	<?php if ($dt_SS == 0 && $sw_error == 0) { ?> // Solución al error del Copiar
		$("#Almacen").change(function(){
			...
			if(document.getElementById('Almacen').value!=""&&document.getElementById('CardCode').value!=""&&document.getElementById('TotalItems').value!="0"){
				Swal.fire({
					title: "¿Desea actualizar el alamcen en las lineas?",
					icon: "question",
					showCancelButton: true,
					confirmButtonText: "Si, confirmo",
					cancelButtonText: "No"
	...
	<?php if ($dt_SS == 0  && $sw_error == 0) { ?> // Solución al error del Copiar
		$("#AlmacenDestino").change(function(){
			...
			if(document.getElementById('AlmacenDestino').value!=""&&document.getElementById('CardCode').value!=""&&document.getElementById('TotalItems').value!="0"){
				Swal.fire({
					title: "¿Desea actualizar el almacen destino en las lineas?",
					icon: "question",
					showCancelButton: true,
					confirmButtonText: "Si, confirmo",
					cancelButtonText: "No"
			  ...
			  <form action="traslado_inventario.php" method="post" class="form-horizontal" enctype="multipart/form-data" id="CrearTrasladoInventario">
				...
				<div class="form-group">
					...
					<div class="col-lg-4">
						...
						<select name="AlmacenDestino" class="form-control select2" id="AlmacenDestino" required <?php if (($edit == 1) && ($row['Cod_Estado'] == 'C')) {
							echo "disabled";
						} ?>>
							<option value="">Seleccione...</option>
						  <?php if ($edit == 1) { ?>
						...
						<select name="Empleado" class="form-control select2" id="Empleado" <?php if (($edit == 1) && ($row['Cod_Estado'] == 'C')) {
							...
							<?php while ($row_Empleado = sqlsrv_fetch_array($SQL_Empleado)) { ?>
										<option value="<?php echo $row_Empleado['ID_Empleado']; ?>" <?php if ((isset($row['CodEmpleado'])) && (strcmp($row_Empleado['ID_Empleado'], $row['CodEmpleado']) == 0)) {
											   echo "selected";
										   } elseif (isset($_GET["Empleado"]) && (base64_decode($_GET["Empleado"]) == $row_Empleado['ID_Empleado'])) {
											   echo "selected";
						...
						<select name="TipoEntrega" class="form-control" id="TipoEntrega" <?php if (($edit == 1) && ($row['Cod_Estado'] == 'C')) {
						  ...
						  <?php while ($row_TipoEntrega = sqlsrv_fetch_array($SQL_TipoEntrega)) { ?>
										<option value="<?php echo $row_TipoEntrega['IdTipoEntrega']; ?>" <?php if ((isset($row['IdTipoEntrega'])) && (strcmp($row_TipoEntrega['IdTipoEntrega'], $row['IdTipoEntrega']) == 0)) {
											   echo "selected";
										   } elseif (isset($_GET["TipoEntrega"]) && (base64_decode($_GET["TipoEntrega"]) == $row_TipoEntrega['IdTipoEntrega'])) {
											   echo "selected";
